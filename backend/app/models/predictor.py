...
class HeatPredictor:
    ...
    def _generate_hotspots(self, city: str, count: int = 15):
        base = self.cities.get(city.lower(), self.cities["mumbai"])
        hotspots = []
        ...
        for i in range(count):
            ...
            hotspots.append({
                "lat": round(base["lat"] + random.uniform(-0.15, 0.15), 6),
                "lng": round(base["lng"] + random.uniform(-0.15, 0.15), 6),
                "lst": lst,
                "ndvi": ndvi,
                "ndbi": ndbi,
                "humidity": humidity,
    ...
    def get_cooling_interventions(self, city: str):
        ...
        for i in range(10):
            ...
            interventions.append({
                "id": f"INT_{i+1:03d}",
                "type": int_type,
                "lat": round(base["lat"] + random.uniform(-0.12, 0.12), 6),
                "lng": round(base["lng"] + random.uniform(-0.12, 0.12), 6),
